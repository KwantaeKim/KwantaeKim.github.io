...
# python=3.11.5 is working fine as of 15-Mar-2024
$ conda install python=3.11.5
```

Here, if you run `search python` then it shows a list of all the possible Python versions. As of 15-Mar-2024, `python=3.11.5` is working fine with VS Code for Jupyter Notebook integration.

Even if there can be newer versions, I suggest using this version as an error can occur because of version conflict. You need to select the python version that is currently compatible with VS code. It keeps causing error otherwise as below:
